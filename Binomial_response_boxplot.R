...
# Load data
load("data_analysis_binomial_GLM_no_RF.RData")
data_analysis_binomial <- data_analysis_binomial

# Color palette interpolation
colourCount <- length(unique(data_analysis_binomial$Collinearity_shift))
...
data_analysis_binomial_high_interaction_GLM <- data_analysis_binomial[data_analysis_binomial$Predictor_coli == "High"&data_analysis_binomial$Range_shift %in% c(0)&
                                                                        data_analysis_binomial$Collinearity_shift %in% c(seq(-.9, .8, by = .1), 0.9)&
                                                                        (data_analysis_binomial$Algorithm == "GLM")&
                                                                        data_analysis_binomial$Model == "Interaction", ]

ggplot(data = data_analysis_binomial_high_interaction_GLM, aes(Range_shift, RMSE, fill = Collinearity_shift)) + 
...
ggplot(data = data_analysis_binomial_high_interaction_GLM, aes(Collinearity_shift, RMSE, fill = Collinearity_shift)) + 
...
ggplot(data = data_analysis_binomial_high_interaction_GLM, aes(Collinearity_shift, RMSE, color = Collinearity_shift)) +
...
vline <- data.frame(Predictor_coli = factor(rep(levels(data_analysis_binomial$Predictor_coli), 1), levels = c(levels(data_analysis_binomial$Predictor_coli))),
...
GLM_col <- ggplot(data = data_analysis_binomial[data_analysis_binomial$Range_shift %in% c(0)&
                                                  data_analysis_binomial$Collinearity_shift %in% c(seq(-.9, .8, by = .1), 0.9)&
                                                  (data_analysis_binomial$Algorithm == "GLM"), ],
...
RF_col <- ggplot(data = data_analysis_binomial[data_analysis_binomial$Range_shift %in% c(0)&
                                                 data_analysis_binomial$Collinearity_shift %in% c(seq(-.9, .8, by = .1), 0.9)&
                                                 (data_analysis_binomial$Algorithm == "RF"), ],
                 aes(Collinearity_shift, RMSE, color = Collinearity_shift, fill = Collinearity_shift)) +
  stat_summary(fun.data = data_summary, geom = "errorbar", width = 0.5, linewidth = 1) +
  stat_summary(fun.data = data_summary_1, geom = "crossbar", width = 0.5, color = "black") + 
  scale_y_continuous(name = "RMSE<br>", labels = formatter(nsmall = 0), breaks = c(0, 1, 2, 3, 4, 5)) +
  scale_x_discrete(breaks = c(seq(-0.9, 0.9, by = 0.2)), name = "Correlation between X<sub>1</sub> and X<sub>2</sub>") +
...
Binomial_GLM_RF_col <- plot_grid(GLM_col, RF_col,
                                 labels = c("(a)", "(b)"),
                                 label_size = 10,
                                 rows = 2,
                                 rel_heights = c(1, 1))
...
ggsave("Binomial_RMSE_GLM_col.png", width = 16, height = 14, units = "cm", dpi = 200)
#unlink("Binomial_RMSE_GLM_RF_col.png")


# Apply the stat_summary with errorbar and cross bar geom to all range shift
# Color palette interpolation
# colourCount_range <- length(unique(data_analysis_binomial$Range_shift))
...
range_GLM <- data_analysis_binomial[data_analysis_binomial$Range_shift %in% c(0, 0.2, 0.4, 0.6, 2, 4, 6)&
                                      data_analysis_binomial$Collinearity_shift %in% c(0.9, 0.6, 0.3)&
                                      (data_analysis_binomial$Algorithm == "GLM"), ]

GLM_nol <- ggplot(data = data_analysis_binomial[data_analysis_binomial$Range_shift %in% c(0, 0.2, 0.4, 0.6, 2, 4, 6)&
                                                  data_analysis_binomial$Collinearity_shift %in% c(0.9, 0.6, 0.3)&
                                                  (data_analysis_binomial$Algorithm == "GLM"), ],
...
RF_nol <- ggplot(data = data_analysis_binomial[data_analysis_binomial$Range_shift %in% c(0, 0.2, 0.4, 0.6, 2, 4, 6)&
                                                 data_analysis_binomial$Collinearity_shift %in% c(0.9, 0.6, 0.3)&
                                                 (data_analysis_binomial$Algorithm == "RF"), ],
                 aes(Range_shift, RMSE, fill = Range_shift)) + 
  stat_summary(fun.data = data_summary, geom = "errorbar", width = 0.5, linewidth = 1) +
...
Binomial_GLM_RF_mol <-  plot_grid(GLM_nol, RF_nol,
                                  labels = c("(a)", "(b)"),
                                  label_size = 10,
                                  rows = 2,
                                  rel_heights = c(1, 1))
...
ggsave("Binomial_RMSE_GLM_nol.png", width = 16, height = 14, units = "cm", dpi = 200)
#unlink("Binomial_RMSE_GLM_RF_nol.png")
