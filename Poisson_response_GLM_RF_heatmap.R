...
# Load data
load("C:/FSU/Collinearity_extra/Revision/Poisson_response/data_analysis_poisson_GLM_no_RF.RData")
...
data_heatmap_GLM <- data_analysis_poisson[data_analysis_poisson$Algorithm == "GLM", ]
...
heatmap_poisson_GLM <- ggplot(data_heatmap_GLM[data_heatmap_GLM$Range_shift %in% c(0, 0.2, 0.4, 0.6), ], mapping = aes(x = as.numeric(as.character(Collinearity_shift)), y = as.numeric(as.character(Range_shift)), z = RMSE)) +
  ...
  metR::geom_contour2(aes(z = RMSE, label = ..level..), binwidth = 0.025, skip = 0, label_size = 3,
...
ggsave("Poisson_RMSE_GLM_heatmap.png", width = 14, height = 14, units = "cm", dpi = 200)
#unlink("Poisson_RMSE_heatmap.png")
...
heatmap_poisson_RF <- ggplot(data_heatmap_RF[data_heatmap_RF$Range_shift %in% c(0, 0.2, 0.4, 0.6), ], mapping = aes(x = as.numeric(as.character(Collinearity_shift)), y = as.numeric(as.character(Range_shift)), z = RMSE)) +
  # geom_tile(aes(fill = RMSE_ratio)) +
  geom_contour_filled(aes(z = RMSE), binwidth = 0.005) +
...
ggsave("Poisson_RMSE_RF_heatmap.png", width = 14, height = 14, units = "cm", dpi = 200)
#unlink("Poisson_RMSE_heatmap.png")

heatmap_poisson_GLM_RF <- plot_grid(heatmap_poisson_GLM, heatmap_poisson_RF,
                                    labels = c("(a)", "(b)"),
                                    label_size = 10,
                                    rows = 2,
                                    rel_heights = c(1, 1))
...
ggsave("Poisson_RMSE_GLM_RF_heatmap.png", width = 16, height = 28, units = "cm", dpi = 200)
#unlink("Poisson_RMSE_heatmap.png")
