...
# Load data
load("C:/FSU/Collinearity_extra/Revision/Binomial_response/data_analysis_binomial_GLM_no_RF.RData")

# Color palettee
library(paletteer)
d  <- paletteer_c("ggthemes::Sunset-Sunrise Diverging", n = 83, direction = 1)

# Calculate the median
data_heatmap_GLM <- data_analysis_binomial[data_analysis_binomial$Algorithm == "GLM", ]
...
heatmap_binomial_GLM <- ggplot(data_heatmap_GLM[data_heatmap_GLM$Range_shift %in% c(0, 0.2, 0.4, 0.6), ], mapping = aes(x = as.numeric(as.character(Collinearity_shift)), y = as.numeric(as.character(Range_shift)), z = RMSE)) +
  # geom_tile(aes(fill = RMSE_ratio)) +
  geom_contour_filled(aes(z = RMSE), binwidth = 0.05) +
  scale_colour_manual(aesthetics = 'fill', drop = FALSE, values = d, name = "Fold change\nof RMSE") +
  # metR::geom_text_contour(aes(z = RMSE), stroke = 0.15) +
  metR::geom_contour2(aes(z = RMSE, label = ..level..), binwidth = 0.5, skip = 0, label_size = 3,
...
ggsave("Binomial_RMSE_GLM_heatmap.png", width = 14, height = 14, units = "cm", dpi = 200)
#unlink("Binomial_RMSE_heatmap.png")


# Calculate the median
data_heatmap_RF <- data_analysis_poisson[data_analysis_poisson$Algorithm == "RF", ]

data_heatmap_RF <- aggregate(cbind(RMSE) ~ Model + Algorithm + Predictor_coli + Range_shift + Collinearity_shift,
                             data = data_heatmap_RF,
                             FUN = median)

heatmap_binomial_RF <- ggplot(data_heatmap_RF[data_heatmap_RF$Range_shift %in% c(0, 0.2, 0.4, 0.6), ], mapping = aes(x = as.numeric(as.character(Collinearity_shift)), y = as.numeric(as.character(Range_shift)), z = RMSE)) +
  # geom_tile(aes(fill = RMSE_ratio)) +
  geom_contour_filled(aes(z = RMSE), binwidth = 0.005) +
  scale_colour_manual(aesthetics = 'fill', drop = FALSE, values = d, name = "Fold change\nof RMSE") +
  # metR::geom_text_contour(aes(z = RMSE), stroke = 0.15) +
  metR::geom_contour2(aes(z = RMSE, label = ..level..), binwidth = 0.5, skip = 0, label_size = 3,
                      label.placer = label_placer_fraction(frac = 0.5, rot_adjuster = isoband::angle_halfcircle_bottom()
...
ggsave("Binomial_RMSE_RF_heatmap.png", width = 14, height = 14, units = "cm", dpi = 200)
#unlink("Binomial_RMSE_heatmap.png")

heatmap_binomial_GLM_RF <- plot_grid(heatmap_binomial_GLM, heatmap_binomial_RF,
                                     labels = c("(a)", "(b)"),
                                     label_size = 10,
                                     rows = 2,
                                     rel_heights = c(1, 1))
...
ggsave("Binomial_RMSE_GLM_RF_heatmap.png", width = 16, height = 28, units = "cm", dpi = 200)
#unlink("Binomial_RMSE_heatmap.png")
