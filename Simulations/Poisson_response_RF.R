# This script was used to simulate the response variable that followed Poisson distributions
...
# Use ranger package for faster model fitting of Random Forests
library(ranger)
...
collineariser_extra <- function(N = 1000, 
  ...
  if (method == "normal"){
    ...
    Ys <- lapply(Data.scaled, function(X) rnorm(n = nrow(X), mean = f(X), sd = runif(1, min = 0, max = 3)))
    Thetas <- lapply(Data.scaled, function(X) f(X))
    
  } else if (method == "poisson") {
    # Calculate Y and theta for Poisson response
    Ys <- lapply(Data.scaled, function(X) rpois(n = nrow(X), lambda = exp(f(X))))
    Thetas <- lapply(Data.scaled, function(X) exp(f(X)))
    
  } else if (method == "binomial") {
    # Calculate Y and theta for Binomial response
    Ys <- lapply(Data.scaled, function(X) rbinom(n = nrow(X), size = 1, prob = plogis(f(X))))
    Thetas <- lapply(Data.scaled, function(X) plogis(f(X)))
  ...
  Data.final[[1]] <- cbind(y = Ys[[1]] + noise, theta = Thetas[[1]], Data.scaled[[1]])
  
  for (i in 1:(length(Data)-1))
    # No noise in the test data
    Data.final[[i+1]] <- cbind(y = Ys[[i+1]], theta = Thetas[[i+1]], Data.scaled[[i+1]])
...
linear_model_2 <- function(vars = c("X1", "X2", "X3"),
  ...
  set.seed(s)
  beta0 <- log(runif(n = 1, min = 0.05, max = 50))
  ...
  fitted <- glm(data = as.data.frame(simulated_data$train), formula, family = poisson(link =log))
  ...
  rmse <- lapply(simulated_data, function(X) (sqrt(mean((predict(fitted, as.data.frame(X), type = "response")-as.data.frame(X)$theta)^2))))
  rrmse <- lapply(simulated_data, function(X) (sqrt(mean(((predict(fitted, as.data.frame(X), type = "response")-as.data.frame(X)$theta)/as.data.frame(X)$theta)^2))))
  ...
  return(list(#"RRMSE" = rrmse, #"RMSE_RF" = rmse_rf,
    ...
    "RRMSE_RF" = rrmse_rf,
    "CN_all" = cn_all,
    "R_square" = model_r_square, #"R_square_rf" = rf_r_square,
...
high_train_coli_interaction <- foreach(icount(1000), .combine = 'c') %dopar% {
  linear_model_2(vars = c("X1", "X2", "X3"),
                 mu = c(0, 0, 0),
                 sd = c(.3, .3, .3),
                 rho = c(0.9, 0.8, 0.7),
                 response = "y",
...
high_inter_rrmse <- setDF(rbindlist(high_train_coli_interaction[names(high_train_coli_interaction) == "RRMSE"]))
high_inter_rrmse
...
# Visualize novelty = 0
plot(seq(0.9, -0.9, by = -0.1), (high_inter_rrmse_mean)[3:21]/(high_inter_rrmse_mean)[2], 
     xlab = "Correlation between X1 and X2", ylab = "RRMSE", main = "Novelty = 0")
plot(seq(0.9, -0.9, by = -0.1), (high_inter_rrmse_median)[3:21]/(high_inter_rrmse_median)[2], 
     xlab = "Correlation between X1 and X2", ylab = "RRMSE", main = "Novelty = 0")

# Versus M distance novelty = 0
plot((high_inter_m_dist_mean)[3:21], (high_inter_rrmse_mean)[3:21]/(high_inter_rrmse_mean)[2], 
     xlab = "Mahalanobis distance", ylab = "rrmse", main = "Novelty = 0")
plot((high_inter_m_dist_median)[3:21], (high_inter_rrmse_median)[3:21]/(high_inter_rrmse_median)[2], 
     xlab = "Mahalanobis distance", ylab = "rrmse", main = "Novelty = 0")
...
# Based on the structure of what foreach returns, subset the lists using list names and rbind all the lists
high_inter_rrmse <- setDF(rbindlist(high_train_coli_inter[names(high_train_coli_inter) == "RRMSE"]))
high_inter_rrmse
...
# Based on the structure of what foreach returns, subset the lists using list names and rbind all the lists
high_quad_rrmse <- setDF(rbindlist(high_train_coli_quad[names(high_train_coli_quad) == "RRMSE"]))
high_quad_rrmse
...
# Based on the structure of what foreach returns, subset the lists using list names and rbind all the lists
high_linear_rrmse <- setDF(rbindlist(high_train_coli_linear[names(high_train_coli_linear) == "RRMSE"]))
high_linear_rrmse
...
# Based on the structure of what foreach returns, subset the lists using list names and rbind all the lists
mid_inter_rrmse <- setDF(rbindlist(mid_train_coli_inter[names(mid_train_coli_inter) == "RRMSE"]))
mid_inter_rrmse
...
# Based on the structure of what foreach returns, subset the lists using list names and rbind all the lists
mid_quad_rrmse <- setDF(rbindlist(mid_train_coli_quad[names(mid_train_coli_quad) == "RRMSE"]))
mid_quad_rrmse
...
# Based on the structure of what foreach returns, subset the lists using list names and rbind all the lists
mid_linear_rrmse <- setDF(rbindlist(mid_train_coli_linear[names(mid_train_coli_linear) == "RRMSE"]))
mid_linear_rrmse
...
# Based on the structure of what foreach returns, subset the lists using list names and rbind all the lists
low_inter_rrmse <- setDF(rbindlist(low_train_coli_inter[names(low_train_coli_inter) == "RRMSE"]))
low_inter_rrmse
...
# Based on the structure of what foreach returns, subset the lists using list names and rbind all the lists
low_quad_rrmse <- setDF(rbindlist(low_train_coli_quad[names(low_train_coli_quad) == "RRMSE"]))
low_quad_rrmse
...
# Based on the structure of what foreach returns, subset the lists using list names and rbind all the lists
low_linear_rrmse <- setDF(rbindlist(low_train_coli_linear[names(low_train_coli_linear) == "RRMSE"]))
low_linear_rrmse
...
# Save the output
save.image("C:/FSU/Collinearity_extra/Revision/Poisson_response/Poisson_response_RF.RData")
