...
linear_model_2 <- function(vars = c("X1", "X2", "X3"),
  ...
  # For Poisson distributions, since there are some extreme large values that > Machine$integer.max
  # Apply repeat loop to control the number of NAs less than 100
  #repeat {
    s <- sample(1:10000, 1)
    ...
    if (length(predictors) == 5) {
      ...
      f <- function(X) ((beta1*X[,"X1"] + beta2*X[,"X1^2"] + beta3*X[,"X2"] + beta4*X[, "X1*X2"] + beta5*X[,"X3"] + beta0)/10)
    } else if (length(predictors) == 4) {
      # Linear and quadratic
      f <- function(X) ((beta1*X[,"X1"] + beta2*X[,"X1^2"] + beta3*X[,"X2"] + beta5*X[,"X3"] + beta0)/10) 
    } else {
      # Linear
      f <- function(X) ((beta1*X[,"X1"] + beta3*X[,"X2"] + beta5*X[,"X3"] + beta0)/10)
    }
    ...
    simulated_data <- collineariser_extra(FUN = f,
                                          ...
                                          empirical = FALSE,
                                          N = 1000)
  #   if(sum(is.na(simulated_data$train$y)) <= 100){
  #    break
  #   }
  # }
  ...
  #                             # Set probability = FALSE for Poisson
  #                             probability = FALSE)
  ...
  # For the calculation of standardized RMSE
  # Note like Pearon residuals (Y - lambda)/sqrt(lambda), a raw residual divided by the standard deviation of unit i
  # To avoid numerical problems when the expected value becomes equal to zero, resulting in a division by zero
  ...
  # RMSE1 = sqrt((sum((Ypred - Yi)/(Yi+e))^2)/n)
  ...
  # RMSE2 = sqrt((sum(Ypred - Yi)^2)/n)/sd((Yi+e))
  ...
  #rf_pred <- predict(fitted_rf, as.data.frame(X))$predictions
  
  # rmse_rf <- lapply(simulated_data, function(X) (sqrt(mean((log(predict(fitted_rf, as.data.frame(X))$predictions + e)-as.data.frame(X)$theta)^2))))
  
  # RMSE1 = sqrt((sum((Ypred - Yi)/(Yi+e)^2)/n)
  # rrmse_rf_1 <- lapply(simulated_data, function(X) (sqrt(mean(((log(predict(fitted_rf, as.data.frame(X))$predictions + e) - as.data.frame(X)$theta)/(as.data.frame(X)$theta + e))^2))))
  
  # RMSE2 = sqrt((sum(Ypred - Yi)^2)/n)/sd((Yi+e))
  # rrmse_rf_2 <- lapply(simulated_data, function(X) ((sqrt(mean((log(predict(fitted_rf, as.data.frame(X))$predictions + e) - as.data.frame(X)$theta)^2)))/(sd((as.data.frame(X)$theta + e)))))
...
# Save the output
save.image("C:/FSU/Collinearity_extra/Revision/Poisson_response/Poisson_response_theta_GLM_no_noise_no_RF.RData")
